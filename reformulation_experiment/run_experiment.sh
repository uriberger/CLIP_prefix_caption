...
venv2/bin/python inference.py ${BASE_DIR}/data/image_ids/image_ids_${EXP_IND}.json ${BASE_DIR}/output/exp_${EXP_IND}_base/coco_prefix-009.pt
mv res_image_ids_${EXP_IND}.json ${BASE_DIR}/data/infer/base_infer_on_train_${EXP_IND}.json
echo "$MSG_PREFIX de -> en"
venv2/bin/python translate.py --source_language de --target_language en --input_file ${BASE_DIR}/data/infer/base_infer_on_train_${EXP_IND}.json --output_file ${BASE_DIR}/data/infer/base_infer_on_train_${EXP_IND}_en.json
cd ../AliceMind/mPLUG
...
venv2/bin/python translate.py --source_language en --target_language de --input_file ${BASE_DIR}/data/infer/base_infer_on_train_${EXP_IND}_en_reformulated.json --output_file ${BASE_DIR}/data/infer/base_infer_on_train_${EXP_IND}_reformulated.json
